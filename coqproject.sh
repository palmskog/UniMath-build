...
echo 'OTHERFLAGS = "-indices-matter -type-in-type"' >> $COQPROJECT_TMP

for dir in ${DIRS[@]}; do
    echo >> $COQPROJECT_TMP
    find $dir -iname '*.v' \
	-not -name '*\#*' \
	-not -name '*SimplicialSets.v' \
	-not -name '*equivalences_lemmas.v' \
	-not -name '*products_from_pullbacks.v' \
	-not -wholename "UniMath/Ktheory/Sets.v" >> $COQPROJECT_TMP
done
